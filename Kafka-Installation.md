...
这是一个为Ubuntu 24.04设计的Kafka完整安装脚本，使用USTC和TUNA镜像源以提高在中国大陆的下载速度。
...
2. 添加执行权限：`chmod +x install_kafka.sh`
3. 运行脚本：`sudo ./install_kafka.sh`

## 脚本说明

- 使用USTC或TUNA镜像源替换默认Ubuntu软件源
- 安装OpenJDK 11（Kafka运行需要Java环境）
- 创建专用kafka用户和所需目录
- 从国内镜像下载最新稳定版Kafka (3.5.2)
- 配置ZooKeeper和Kafka服务
- 设置系统服务，实现开机自启
- 创建测试主题验证安装
...
1. 此脚本配置为单节点模式，适合开发测试环境
2. 生产环境应考虑配置集群模式并优化相关参数
3. 默认数据保留期为7天（168小时）
4. 主题删除功能已启用（delete.topic.enable=true）
5. 配置中已包含基本安全和性能优化设置
...
sudo rm /etc/systemd/system/kafka.service /etc/systemd/system/zookeeper.service
sudo systemctl daemon-reload
sudo rm -rf /opt/kafka /var/lib/kafka /var/log/kafka /var/lib/zookeeper /var/log/zookeeper
