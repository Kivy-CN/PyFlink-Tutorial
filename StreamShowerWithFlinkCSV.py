...
def read_from_kafka():
    ...
    env.add_source(kafka_consumer).map(lambda x: ' '.join(re.findall(r'\d+', x))).filter(lambda x: any([1900 <= int(i) <= 2023 for i in x.split()])).map(lambda x: '3> ' + [i for i in x.split() if 1900 <= int(i) <= 2023][0]).print()
